# （归档）OpenClaw × Claude Code × tmux：把 AI 执行从"开窗口"变成"管作业"
...
这套方案默认是单机编排；如果你扩展到远程执行（进阶），它的本质会升级为**远程代码执行**。必须做到：

- 若使用远程执行（进阶），SSH key 限制最小必要权限
